...
public class CatsTests
{
       ...
       _data = new List<Cat>
    {
       ...
       _mockSet.As<IQueryable<Cat>>().Setup(m => m.Provider).Returns(_data.AsQueryable().Provider);
       _mockSet.As<IQueryable<Cat>>().Setup(m => m.Expression).Returns(_data.AsQueryable().Expression);
        _mockSet.As<IQueryable<Cat>>().Setup(m => m.ElementType).Returns(_data.AsQueryable().ElementType);
        _mockSet.As<IQueryable<Cat>>().Setup(m => m.GetEnumerator()).Returns(_data.GetEnumerator());
